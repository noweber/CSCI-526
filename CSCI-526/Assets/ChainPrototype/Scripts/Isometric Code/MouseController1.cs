...
public class MouseController1 : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        ...
        if (hit.HasValue)
        {
            ...
            {
                if (character.unitName != "Triangle")
                {
            ...
            {
                tile.ShowTile();
                if (character != null && character.standingOnTile.gridLocation == tile.gridLocation)
                {
                    character = null;
                    isMoving = false;
                    tile.HideTile();
                }
                else if (MapManager.Instance.playerLocations.Contains(tile.gridLocation) && character==null)
                {
                    //Debug.Log("DETECTION: " + tile.gridLocation);
                    foreach (var player in MapManager.Instance.players)
                    {
                        ...
                        if (player.gameObject.GetComponent<CharacterInfo>().standingOnTile.gridLocation == tile.gridLocation)
                        {
                            character = player;
                            break;
                        }
                    ...
                    {
                        Debug.Log("Triangle Ability Pop up");
                    }
