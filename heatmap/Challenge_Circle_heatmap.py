...
def main():
    ...
    for row_data in data[1:]:
        if(row_data[3]== 'Challenge_Circle'):
            hashmap = json.loads(row_data[13])
            if(hashmap['VictoriousPlayerName'] == 'AI' ):    #hashmap['VictoriousPlayerName'] == 'Human'
    ...
    heatmap_cc = np.zeros((6,5), dtype=int)
    for i in player_movement:
        x = int(i[0])
        y = int(i[1])
        heatmap_cc[6-y-1,x] += int(player_movement[i])
    ...
    ax.set_yticks(np.arange(6), labels = ylabel[4:] )
    ax.set_xticks(np.arange(5), labels = xlabel[0:5])
    ax.set_title("Challenge_Circle Player HeatMap")
